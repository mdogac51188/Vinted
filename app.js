...
/* =========================
   DEBUG OVERLAY (zeigt JS-Fehler direkt im Browser)
   ========================= */
(function(){
  try{
    const id = "mm_debug_overlay";
    const el = document.createElement("div");
    el.id = id;
    el.style.cssText = "position:fixed;left:10px;bottom:10px;z-index:99999;max-width:90vw;background:rgba(0,0,0,.75);color:#fff;font:12px/1.35 system-ui;padding:10px 12px;border-radius:12px;display:none;white-space:pre-wrap;";
    document.addEventListener("DOMContentLoaded", ()=>{ document.body && document.body.appendChild(el); });
    function show(msg){
      const box = document.getElementById(id) || el;
      box.textContent = "Fehler in app.js:\n" + String(msg || "");
      box.style.display = "block";
    }
    window.addEventListener("error", (e)=>{ show(e?.message || e); }, true);
    window.addEventListener("unhandledrejection", (e)=>{ show(e?.reason?.message || e?.reason || e); }, true);
  }catch{}
})();
...
function parseEUR(v) {
  // Accept number or strings like "18,95", "18.95", "18,95 €"
  if (v == null) return 0;
...
async function initIndex() {
  ...
  function render() {
    ...
    list.forEach(l => {
      const raw0 = (Array.isArray(l.images) && typeof l.images[0] === "string") ? l.images[0] : "";
      const src0 = (!raw0 || raw0.startsWith("blob:")) ? svgPlaceholder(l.title || l.category || "Artikel") : raw0;
      const imgHtml = `<img src="${src0}" alt="">`;
      const el = document.createElement("div");
      el.className = "item";
      el.innerHTML = `
...
/* =========================
   PAGES: SELL (FIX Bilder: DataURL statt blob:)
   ========================= */
async function initSell() {
  ...
  // Preview: lokal okay (blob), aber nur fürs UI – wird nach Load revoked
  function renderPreview() {
    ...
    for (const f of files.slice(0, 4)) {
      ...
      if (im) im.onload = () => { try { URL.revokeObjectURL(url); } catch {} };
      preview.appendChild(div);
    }
  }

  async function fileToJpegDataUrl(file, maxW = 1100, maxH = 1100, quality = 0.82) {
  ...
  async function filesToImages(files, maxCount = 4) {
    const out = [];
    const list = Array.from(files || []).slice(0, maxCount);
    for (const f of list) {
      if (!f || !String(f.type || "").startsWith("image/")) continue;

      // Schutz: localStorage kann schnell voll werden
      if (Number(f.size || 0) > 6 * 1024 * 1024) {
        alert("Bild zu groß (max 6 MB). Bitte kleineres Bild wählen.");
        continue;
      }

      try {
        const d = await fileToJpegDataUrl(f);
        if (d) out.push(d);
      } catch (e) {
  ...
  $("#sellForm")?.addEventListener("submit", async (e) => {
    ...
    const category = String($("#category")?.value || "").trim() || "Sonstiges";

    // Preis mit Komma erlauben
    const rawPrice = String($("#price")?.value || "").trim().replace(",", ".");
    const price = Number(rawPrice);

    const city = String($("#city")?.value || u.city || "").trim();
    const description = String($("#desc")?.value || "").trim();

    if (!title) { alert("Titel fehlt"); return; }
    if (!(price > 0)) { alert("Preis ungültig"); return; }

    let images = await filesToImages(imgInput?.files, 4);
    if (!images.length) images = [svgPlaceholder(title || "Artikel")];
    ...
    try {
      ...
      console.warn(err);
      alert("Speichern fehlgeschlagen (Speicher voll). Bitte kleinere Bilder/ weniger Bilder verwenden.");
    }
  });
}
...
async function initListing() {
  ...
  if (img) {
    const raw0 = (Array.isArray(l.images) && typeof l.images[0] === "string") ? l.images[0] : "";
    const src0 = (!raw0 || raw0.startsWith("blob:")) ? svgPlaceholder(l.title || l.category || "Artikel") : raw0;
    img.innerHTML = `<img src="${src0}" alt="">`;
  }
  ...
  if (actions) {
    ...
    const isSellerView = !!(u0 && String(u0.id) === String(l.sellerId));
...
$("#btnCart")?.addEventListener("click", () => addToCart(l.id));
    ...
    $("#btnChat")?.addEventListener("click", () => {
      ...
      // Seller: Chat mit Buyer (aus Order-Param oder Reservierung)
      if (String(u.id) === String(l.sellerId)) {
        const oid = qs("order");
        const o = oid ? getOrder(oid) : null;
        const otherId = (o && o.buyerId) ? o.buyerId : (l.reservedBy || "");
        if (!otherId) { location.href = "messages.html"; return; }
        location.href = `messages.html?with=${encodeURIComponent(otherId)}&listing=${encodeURIComponent(l.id)}`;
        return;
      }

      // Buyer: Chat mit Seller
      location.href = `messages.html?with=${encodeURIComponent(l.sellerId)}&listing=${encodeURIComponent(l.id)}`;
    });
  ...
  if (btnBuy) {
    ...
    const urlOrderId = qs("order");
    const urlOrder = urlOrderId ? getOrder(urlOrderId) : null;

    function findLatestOrderForListing() {
      const list = orders().filter(o => String(o.listingId) === String(l.id));
      list.sort((a,b)=>(b.createdAt||0)-(a.createdAt||0));
      return list[0] || null;
    }

    function findMyPendingOrder() {
      if (!u0) return null;
      const list = orders().filter(o =>
        String(o.listingId) === String(l.id) &&
        String(o.buyerId) === String(u0.id) &&
    ...
    function ensurePendingForReservedBuyer() {
      if (!u0) return null;
      const fresh = getListing(l.id);
      if (!fresh) return null;

      const st = String(fresh.status || "").toUpperCase();
      if (st === "ACTIVE") return null;

      if (st === "RESERVED" && String(fresh.reservedBy || "") === String(u0.id)) {
        const existing = findMyPendingOrder();
    ...
    let mode = "NONE"; // BUY | PAY | SHIP | NONE
    let modeOrderId = "";

    if (u0 && String(u0.id) === String(l.sellerId)) {
      // Seller-Ansicht: bei bezahlten Orders -> "Artikel verschicken"
      const o = (urlOrder && String(urlOrder.sellerId) === String(u0.id)) ? urlOrder : findLatestOrderForListing();

      if (o && ["PAID","SHIPPED","COMPLETED"].includes(String(o.status || "").toUpperCase())) {
        mode = "SHIP";
        modeOrderId = o.id;
        btnBuy.disabled = false;
        btnBuy.textContent = (String(o.status || "").toUpperCase() === "PAID") ? "Artikel verschicken" : "Auftrag öffnen";
      } else {
        btnBuy.disabled = true;
        btnBuy.textContent = "Nicht verfügbar";
      }
    } else {
      // Buyer / nicht eingeloggt
      let pending = null;

      if (u0) {
        if (urlOrder && String(urlOrder.buyerId) === String(u0.id) && String(urlOrder.status || "").toUpperCase() === "PENDING_PAYMENT") {
          pending = urlOrder;
        }
        if (!pending) pending = findMyPendingOrder();
        if (!pending && lStatus === "RESERVED" && String(l.reservedBy || "") === String(u0.id)) {
          pending = ensurePendingForReservedBuyer();
        }
      }

      if (pending && u0) {
        mode = "PAY";
        modeOrderId = pending.id;
        btnBuy.disabled = false;
        btnBuy.textContent = "Jetzt bezahlen";
      } else if (lStatus === "ACTIVE") {
        mode = "BUY";
        btnBuy.disabled = false;
        btnBuy.textContent = "Kaufen";
      } else if (lStatus === "RESERVED") {
        btnBuy.disabled = true;
        btnBuy.textContent = "Reserviert";
      } else if (lStatus === "SOLD") {
        btnBuy.disabled = true;
        btnBuy.textContent = "Verkauft";
      } else {
        btnBuy.disabled = true;
        btnBuy.textContent = "Nicht verfügbar";
      }
    ...
    // Bind once
    if (btnBuy.getAttribute("data-buybind") !== "1") {
      btnBuy.setAttribute("data-buybind", "1");
      btnBuy.addEventListener("click", () => {
        ...
        // Seller: Order öffnen
        if (String(u.id) === String(l.sellerId)) {
          if (mode === "SHIP" && modeOrderId) {
            location.href = `order.html?id=${encodeURIComponent(modeOrderId)}`;
          }
          return;
        }

        // Buyer: Pay
        if (mode === "PAY" && modeOrderId) {
          location.href = `checkout.html?order=${encodeURIComponent(modeOrderId)}`;
          return;
        }
...
async function initCheckout() {
  const u = currentUser();
  if (!u) {
    const next = "checkout.html" + (location.search || "");
  ...
  // POS25: Versandart + Gesamt (speichert in Order)
  const shipOptions = [
    { id: "pickup", label: "Abholung (0,00 €)", cost: 0 },
    { id: "dhl",    label: "DHL Paket (4,99 €)", cost: 4.99 },
    { id: "hermes", label: "Hermes Paket (4,49 €)", cost: 4.49 }
  ];
  const getShipOpt = (id) => shipOptions.find(x => x.id === id) || shipOptions[0];

    // POS36: robust money parsing (fixes 0,00€ when values are strings like "18,95 €")
  const listingPrice = parseEUR(l?.price ?? l?.priceEUR ?? l?.priceCents);
  const oTotal = parseEUR(o.total);
  const oShip = (o.shippingCost != null) ? parseEUR(o.shippingCost) : null;
  const oSub = (o.subtotal != null) ? parseEUR(o.subtotal) : null;
  let subtotal = (oSub != null && oSub > 0) ? oSub : (listingPrice > 0 ? listingPrice : (oShip != null ? Math.max(0, oTotal - oShip) : oTotal));
  if (!isFinite(subtotal) || subtotal < 0) subtotal = 0;

  const setTotalsUI = (shippingCost) => {
    const total = Math.round((Number(subtotal) + Number(shippingCost || 0)) * 100) / 100;
    $("#coSubtotal") && ($("#coSubtotal").textContent = moneyEUR(subtotal));
    $("#coShipping") && ($("#coShipping").textContent = moneyEUR(shippingCost || 0));
    $("#coTotal") && ($("#coTotal").textContent = moneyEUR(total));
  };

  const saveShipping = (methodId) => {
    const opt = getShipOpt(String(methodId || "pickup"));
    const shippingCost = parseEUR(opt.cost || 0);
    const total = Math.round((Number(subtotal) + shippingCost) * 100) / 100;

    updateOrderById(o.id, {
      subtotal,
    ...
    setTotalsUI(shippingCost);
  };

  // Initial anzeigen (ohne sofort zu speichern, außer wenn noch nichts gesetzt ist)
  const initialMethod = String(o.shippingMethod || "pickup");
  const initialOpt = getShipOpt(initialMethod);
  const initialShipCost = (o.shippingCost != null) ? Number(o.shippingCost) : Number(initialOpt.cost || 0);
  setTotalsUI(initialShipCost);

  if (o.shippingMethod == null && o.shippingCost == null && o.subtotal == null) {
    // Default einmalig speichern, damit F5 / später stabil ist
    updateOrderById(o.id, {
      subtotal,
      shippingMethod: initialMethod,
      shippingCost: initialShipCost,
      total: Math.round((Number(subtotal) + Number(initialShipCost || 0)) * 100) / 100
    });
  }

  // Versandoptionen rendern
  const shipWrap = $("#coShipOptions");
  if (shipWrap) {
    const locked = (o.status !== "PENDING_PAYMENT");
    shipWrap.innerHTML = shipOptions.map(opt => {
      const checked = (opt.id === initialMethod) ? "checked" : "";
      const dis = locked ? "disabled" : "";
      return `
        <label style="display:flex;align-items:center;gap:8px;cursor:pointer;">
          <input type="radio" name="shipMethod" value="${opt.id}" ${checked} ${dis} />
          <span>${escapeHtml(opt.label)}</span>
        </label>
      `;
    }).join("");

    shipWrap.addEventListener("change", (ev) => {
      const t = ev.target;
      if (!t || t.name !== "shipMethod") return;
      const fresh = getOrder(o.id);
      if (!fresh || fresh.status !== "PENDING_PAYMENT") return;
      saveShipping(String(t.value));
    });
  }

// POS36_ADDR_REQUIRED: Adresse muss immer ausgefüllt sein, sonst bleibt Bezahlen deaktiviert
  const addrIds = ["coAddrName","coAddrStreet","coAddrZip","coAddrCity","coAddrCountry"];
  const getAddr = () => ({
    name: String($("#coAddrName")?.value || "").trim(),
    street: String($("#coAddrStreet")?.value || "").trim(),
    zip: String($("#coAddrZip")?.value || "").trim(),
    city: String($("#coAddrCity")?.value || "").trim(),
    country: String($("#coAddrCountry")?.value || "Deutschland").trim() || "Deutschland"
  });
  const validateAddr = (a) => {
    if (!a.name || a.name.length < 2) return "Bitte Name eingeben.";
    if (!a.street || a.street.length < 4) return "Bitte Straße + Hausnummer eingeben.";
    if (!/^\d{5}$/.test(a.zip || "")) return "PLZ muss 5-stellig sein (z.B. 12345).";
    if (!a.city || a.city.length < 2) return "Bitte Stadt eingeben.";
    return "";
  };
  const refreshPayState = () => {
    ...
    if (!btn) return;
    const a = getAddr();
    const msg = validateAddr(a);
    if (err) err.textContent = msg ? ("⚠️ " + msg) : "";
    const fresh = getOrder(o.id);
    const pending = !!fresh && fresh.status === "PENDING_PAYMENT";
    btn.disabled = !pending || !!msg;
    if (!msg) updateOrderById(o.id, { shippingAddress: a });
  };
  ...
  addrIds.forEach(id => {
    ...
    el.disabled = false;
    el.readOnly = false;
    el.addEventListener("input", refreshPayState);
  });
...
async function initOrder() {
  ...
  const l = getListing(o.listingId);
// POS36_ORDER_BREAKDOWN: fix 0,00€ by parsing strings and using listing price as fallback
  const totalN = parseEUR(o.total);
  const shipN = parseEUR(o.shippingCost);
  let subN = parseEUR(o.subtotal);
  const lpN = parseEUR(l?.price ?? l?.priceEUR ?? l?.priceCents);
  if (!subN || subN <= 0) {
    subN = lpN > 0 ? lpN : Math.max(0, totalN - shipN);
  }
  const shipFixed = (shipN > 0) ? shipN : Math.max(0, totalN - subN);
  $("#oSubtotal") && ($("#oSubtotal").textContent = moneyEUR(subN));
  $("#oShipping") && ($("#oShipping").textContent = moneyEUR(shipFixed));
  ...
  $("#oTotal") && ($("#oTotal").textContent = moneyEUR(parseEUR(o.total)));
...
async function initProfile() {
  ...
  // ✅ Meine Inserate: verkaufte Inserate ausblenden (nach Kauf nicht mehr anzeigen)
  const soldIds = new Set(
    orders()
      .filter(o => o.sellerId === u.id && ["PAID","SHIPPED","COMPLETED"].includes(String(o.status||"").toUpperCase()))
      .map(o => String(o.listingId))
  );
  const myL = allListings().filter(l =>
    l.sellerId === u.id &&
    String(l.status || "").toUpperCase() !== "SOLD" &&
    !soldIds.has(String(l.id))
  );
  ...
  const myS = orders().filter(o =>
    o.sellerId === u.id &&
    !["CANCELED","CANCELLED","DECLINED","REJECTED"].includes(String(o.status || "").toUpperCase())
  ...
  function ensureSellerDash(){
    ...
    function render(){
      ...
      listEl.innerHTML = list.map(o=>{
        ...
        const raw0 = (Array.isArray(l?.images) && typeof l.images[0] === "string") ? l.images[0] : "";
        const src0 = (!raw0 || raw0.startsWith("blob:")) ? svgPlaceholder(title) : raw0;
          ...
          <div class="card" style="padding:12px;margin:10px 0;">
            <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start;flex-wrap:wrap;">
              <div style="display:flex;gap:12px;align-items:flex-start;flex:1;min-width:260px;">
                <div style="width:86px;flex:0 0 86px;">
                  <img src="${src0}" alt="" style="width:86px;height:64px;object-fit:cover;border-radius:12px;display:block;" />
                </div>
...
/* =========================
   POS27_IMG_FIX: ensure listing image shows even if CSS hides <img>
   - only affects listing page (data-page="listing")
   - sets BOTH background-image + <img> with inline styles
   ========================= */
(function () {
  function byId(id){ return document.getElementById(id); }
  function qsParam(name){
    try { return (typeof qs === "function") ? qs(name) : (new URLSearchParams(location.search).get(name)); }
    catch { return null; }
  }
  function getListingSafe(id){
    try { return (typeof getListing === "function") ? getListing(id) : null; } catch { return null; }
  }
  function svgPlaceholderSafe(text){
    try { return (typeof svgPlaceholder === "function") ? svgPlaceholder(text) : ""; } catch { return ""; }
  }
  function applyImgBox(box, src){
    if (!box) return;
    var s = String(src || "");
    if (!s) return;

    // Background fallback (works even if img tags are hidden in CSS)
    box.style.backgroundImage = 'url("' + s.replace(/"/g, '\\"') + '")';
    box.style.backgroundSize = "cover";
    box.style.backgroundPosition = "center";
    box.style.backgroundRepeat = "no-repeat";

    // Also inject <img> (works if background is not styled)
    box.innerHTML = '<img src="' + s.replace(/"/g, '\\"') + '" alt="" style="width:100%;height:100%;object-fit:cover;display:block;border-radius:inherit;">';
  }
  ...
  document.addEventListener("DOMContentLoaded", function(){
    try{
      ...
      var id = qsParam("id");
      var l = getListingSafe(id);
      if (!l) return;

      var raw0 = (Array.isArray(l.images) && typeof l.images[0] === "string") ? l.images[0] : "";
      var src0 = (!raw0 || String(raw0).startsWith("blob:")) ? svgPlaceholderSafe(l.title || l.category || "Artikel") : raw0;

      applyImgBox(byId("lImage"), src0);
    } catch (e) {
      console.error("POS27_IMG_FIX error", e);
    }
  });
})();

/* =========================
   POS29_SHIPPING_COST: seller sets per-listing shipping cost; buyer sees on listing; checkout uses it
   - safe / additive: does not change existing flows unless page matches
   - stores: listing.shippingMode ("both"|"ship"|"pickup"), listing.shippingCost (number)
   ========================= */
(function(){
  // --- tiny helpers
  function page(){ try { return document.body && document.body.getAttribute("data-page"); } catch(e){ return ""; } }
  function $(sel){ return document.querySelector(sel); }
  function byId(id){ return document.getElementById(id); }
  function money(v){ return (typeof moneyEUR === "function") ? moneyEUR(v) : (Number(v||0).toFixed(2) + " €"); }
  function toast(msg, type, ms){ return window.mmToast ? window.mmToast(msg, type || "ok", ms || 1400) : alert(msg); }
  function toNum(x){ var n = Number(String(x||"").replace(",", ".")); return isFinite(n) ? n : NaN; }
  function qsParam(name){
    try { return (typeof qs === "function") ? qs(name) : (new URLSearchParams(location.search).get(name)); }
    catch(e){ return null; }
  }
  function isBatchMode(){
    try { return (new URLSearchParams(location.search).get("batch") === "1"); } catch(e){ return false; }
  }

  // --- SELL PAGE: inject UI + validate
  function ensureSellShippingUI(){
    const form = byId("sellForm");
    if (!form) return;

    // Already injected?
    if (byId("shipMode") || byId("shipCost")) return;

    const block = document.createElement("div");
    block.id = "shipBlock";
    block.innerHTML = `
      <div class="hr"></div>
      <div class="small-muted" style="font-weight:900;margin-bottom:6px;">Versand / Abholung</div>
      <select class="input" id="shipMode">
        <option value="both">Versand + Abholung</option>
        <option value="ship">Nur Versand</option>
        <option value="pickup">Nur Abholung</option>
      </select>
      <input class="input" id="shipCost" placeholder="Versandkosten (€)" inputmode="decimal" value="4.99" />
      <div class="small-muted" style="opacity:.9;margin-top:6px;">
        Tipp: Bei „Nur Abholung“ werden Versandkosten automatisch 0 gesetzt.
      </div>
    `;

    // Insert before images section if possible, else before submit button
    const images = byId("images");
    if (images && images.parentNode) {
      form.insertBefore(block, images.previousElementSibling ? images.previousElementSibling : images);
    } else {
      const btn = form.querySelector('button[type="submit"]');
      form.insertBefore(block, btn || null);
    }

    function refresh(){
      const mode = String(byId("shipMode")?.value || "both");
      const costEl = byId("shipCost");
      if (!costEl) return;
      if (mode === "pickup") {
        costEl.value = "0";
        ...
        if (String(costEl.value || "").trim() === "") costEl.value = "4.99";
      }
    }
    byId("shipMode")?.addEventListener("change", refresh);
    refresh();

    // Validate on submit (capture, so we stop invalid before existing handler runs)
    form.addEventListener("submit", function(e){
      try{
        const mode = String(byId("shipMode")?.value || "both");
        const costEl = byId("shipCost");
        const cost = toNum(costEl ? costEl.value : "");
        if (mode !== "pickup") {
          if (!isFinite(cost) || cost < 0) {
            e.preventDefault();
            e.stopImmediatePropagation();
            toast("⚠️ Bitte gültige Versandkosten eingeben (z.B. 4.99).", "warn", 1700);
            return;
          }
        }
      } catch(err) {
        // do nothing; allow normal submit
      }
    }, true);
  }

  // --- STORE into listing after createListing (wrapper, idempotent)
  function installCreateListingWrapper(){
    if (window.__mm_ship_wrap_v1) return;
    window.__mm_ship_wrap_v1 = true;

    if (typeof createListing !== "function" || typeof listings !== "function" || typeof setListings !== "function") return;

    const _orig = createListing;
    createListing = function(data){
      const l = _orig(data);

      try{
        // Prefer explicit args, else read from sell UI
        let mode = (data && data.shippingMode) ? String(data.shippingMode) : "";
        let cost = (data && data.shippingCost != null) ? Number(data.shippingCost) : NaN;

        if (!mode && page() === "sell") mode = String(byId("shipMode")?.value || "both");
        if (!isFinite(cost) && page() === "sell") cost = toNum(byId("shipCost")?.value);

        // Defaults for old usage
        if (!mode) mode = "both";
        if (!isFinite(cost)) cost = (mode === "pickup") ? 0 : 4.99;
        if (mode === "pickup") cost = 0;

        // Persist into stored listing
        const list = listings();
        const idx = list.findIndex(x => x && x.id === l.id);
        if (idx >= 0) {
          list[idx] = { ...list[idx], shippingMode: mode, shippingCost: Number(cost) };
          setListings(list);
        }
        // Also on returned object
        l.shippingMode = mode;
        l.shippingCost = Number(cost);
      } catch(e) {}
  ...
  // --- LISTING PAGE: show shipping info before buy
  function showListingShipping(){
    const id = qsParam("id");
    if (!id || typeof getListing !== "function") return;
    const l = getListing(id);
    if (!l) return;

    // Build text
    const mode = String(l.shippingMode || "both");
    const cost = (l.shippingCost != null) ? Number(l.shippingCost) : 4.99;

    let text = "";
    if (mode === "pickup") text = "Abholung (0,00 €)";
    else if (mode === "ship") text = "Versand: " + money(cost);
    else text = "Versand: " + money(cost) + " · Abholung möglich";

    // Insert line if missing
    let el = byId("lShipInfo");
    if (!el) {
      const priceEl = byId("lPrice");
  ...
  // --- CHECKOUT PAGE: use per-listing shipping cost for DHL/Hermes (same value), pickup free
  function applyCheckoutShippingFromListing(){
    if (isBatchMode()) return; // keep POS16 batch flow untouched
    const wrap = byId("coShipOptions");
    if (!wrap) return;

    const orderId = qsParam("order");
    if (!orderId || typeof getOrder !== "function") return;
    const o = getOrder(orderId);
    if (!o) return;
    if (typeof getListing !== "function") return;
    const l = getListing(o.listingId);
    if (!l) return;

    const mode = String(l.shippingMode || "both");
    const baseShip = (l.shippingCost != null && isFinite(Number(l.shippingCost))) ? Number(l.shippingCost) : 4.99;

    function updateLabels(){
      const radios = wrap.querySelectorAll('input[name="shipMethod"]');
      radios.forEach(r => {
        const v = String(r.value || "");
        const labelSpan = r.parentElement ? r.parentElement.querySelector("span") : null;
        if (!labelSpan) return;

        if (v === "pickup") {
          labelSpan.textContent = "Abholung (0,00 €)";
          r.disabled = (mode === "ship"); // only shipping -> disable pickup
        } else if (v === "dhl") {
          labelSpan.textContent = "DHL Paket (" + money(baseShip) + ")";
          r.disabled = (mode === "pickup");
        } else if (v === "hermes") {
          labelSpan.textContent = "Hermes Paket (" + money(baseShip) + ")";
          r.disabled = (mode === "pickup");
        }
      });

      // If pickup-only: force pickup
      if (mode === "pickup") {
        const pick = wrap.querySelector('input[name="shipMethod"][value="pickup"]');
        if (pick) pick.checked = true;
      }
    }

    function subtotalGuess(){
      // Prefer order.subtotal, else (order.total - order.shippingCost), else listing price
      const shipCost = (o.shippingCost != null) ? Number(o.shippingCost) : 0;
      if (o.subtotal != null) return Number(o.subtotal) || 0;
      if (o.total != null) return Math.max(0, Number(o.total || 0) - shipCost);
      return Number(l.price || 0);
    }

    function setUI(sub, ship){
      const tot = Math.round((Number(sub||0) + Number(ship||0)) * 100) / 100;
      byId("coSubtotal") && (byId("coSubtotal").textContent = money(sub));
      byId("coShipping") && (byId("coShipping").textContent = money(ship));
      byId("coTotal") && (byId("coTotal").textContent = money(tot));
    }

    function costForMethod(m){
      m = String(m||"pickup");
    ...
    // compute + sync order
    const sub = subtotalGuess();
    const checked = wrap.querySelector('input[name="shipMethod"]:checked');
    const method = checked ? String(checked.value||"pickup") : (mode === "pickup" ? "pickup" : "dhl");
    const shipCost = costForMethod(method);

    if (typeof updateOrderById === "function") {
      updateOrderById(o.id, { subtotal: sub, shippingMethod: method, shippingCost: shipCost, total: Math.round((sub + shipCost)*100)/100 });
    }
    setUI(sub, shipCost);

    // intercept changes (capture) so old listener can't overwrite costs
    wrap.addEventListener("change", function(e){
      ...
      const m = String(t.value || "pickup");
      const ship = costForMethod(m);
      const sub2 = subtotalGuess();

      if (typeof updateOrderById === "function") {
        updateOrderById(o.id, { subtotal: sub2, shippingMethod: m, shippingCost: ship, total: Math.round((sub2 + ship)*100)/100 });
      }
      setUI(sub2, ship);
    }, true);
  }
  ...
  document.addEventListener("DOMContentLoaded", function(){
    try{
      if (page() === "sell") ensureSellShippingUI();
      installCreateListingWrapper();

      if (page() === "listing") showListingShipping();
      if (page() === "checkout") applyCheckoutShippingFromListing();
    } catch(e) {
      console.error("POS29 shipping patch error", e);
    }
  });
})();
